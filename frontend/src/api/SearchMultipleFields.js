 ...
 */
function SearchMultipleFields (searchTerm = '*', searchFields = ['*']) {
  let searchResults = []

  searchFields.forEach(field => {
    const employeeSearch = Search.employees(field, searchTerm)
      .then(response => {
        let employeeResults = response.response.docs.length > 0 ? response.response.docs : []
        return employeeResults
      })
    const departmentSearch = Search.departments(field, searchTerm)
      .then(response => {
